...
#ifndef SF_CTSU_COMM_API_H_
#define SF_CTSU_COMM_API_H_
...
Typedef definitions
***********************************************************************************************************************/
typedef void sf_ctsu_comm_ctrl_t;

typedef struct st_sf_ctsu_comm_cfg
{
    uint32_t index;
...
typedef struct st_sf_ctsu_comm_api
{
     ...
     */
    ssp_err_t (* open)(sf_ctsu_comm_ctrl_t       * const p_ctrl,
            sf_ctsu_comm_cfg_t  const * const p_cfg);

    /** Clean up Tuning driver.
     * @param[in]   p_ctrl      Pointer to device control block initialized in Open call for Tuning driver.
     */
    ssp_err_t (* close)(sf_ctsu_comm_ctrl_t * const p_ctrl);

    /**
     * Run the Tuning driver
     * @param[in] p_ctrl      Pointer to device control block initialized in Open call for Tuning driver.
     */
    ssp_err_t (* run)(sf_ctsu_comm_ctrl_t * const p_ctrl);

}sf_ctsu_comm_api_t;

typedef struct st_sf_ctsu_comm_instance
{
    sf_ctsu_comm_ctrl_t       * p_ctrl;
    sf_ctsu_comm_api_t  const * p_api;
    sf_ctsu_comm_cfg_t  const * p_cfg;
}sf_ctsu_comm_instance_t;
